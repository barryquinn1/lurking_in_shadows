...
abstract: "This paper interrogates the impact of the Kyoto Protocol Agreement (KPA) target setting regime on carbon *shadow* pricing and environmental efficiency. We extract shadow price estimates and efficiency scores from a comprehensive dataset of 125 countries over the four years of the KPA using the unified StoNED framework. We exploit recent work, which uses the *least-cost alternative* procedure to estimate CO~2~ marginal abatement costs from initial model estimation. Results reveal abatement costs are significantly higher for target setting countries, increase over the sample period, and are an order of magnitude greater than the prevailing emissions pricing mechanisms. Typically, environmental inefficiency scores are higher in countries with higher trade to GDP ratios, higher urban density, and explicit C0~2~ emission targets set under the Kyoto Protocol Agreement. Our findings provide insights into the consequences of policies to curb unwanted byproducts in a regulated system and reveal *future price* consequences of the regulation used to mitigate undesirable outputs."
