...
text_tbl %>% kable("latex",caption = "Description of model variables", row.names = T,align = "c",booktabs=T) %>% kable_styling(latex_options = c("HOLD_position","scale_down"))
```

Table provides a detailed description of the modelling variables. We use the International Energy Association (IEA) database[^10] which provides the most extensive global coverage of CO2 emission data. This database estimates CO2 from fuel emission measured in Metric Tonnes for rover 140 countries from 1960 to 2016. After removing countries with missing observations, we have a balanced sample of 525 observations for 2008-2012. Figure @fig-map visualises the countries geography of the countries in our sample, colour coding the target[^11] setting and non-target[^12] setting nations. The grey shaded areas are countries that where not part of the KP.
